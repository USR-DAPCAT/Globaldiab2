...
```{r generacio_dtindex2, include=F, eval=FALSE}
##Nota SIDIAP:
...
dt_diagnostics %>% semi_join(dt_cataleg2,by="cod") %>% group_by(idp) %>% slice(which.min(dat)) %>% ungroup() %>% select(idp,dtindex=dat)
...
```{r generacio_dtindex3, include=F, eval=FALSE}
# - Condició b) Prescripció de com a mínim 3 mesos glucose-lowering medication una vegada definit l’agregador
#1.	Temps acumulat de prescripció > 3mesos  
#2.	3 envasos de facturació 
...
temp_facturacions2<- 
  temp_facturacions %>% agregar_facturacio(finestra.dies = c(-Inf,+Inf),
                                     ...
                                     prefix = "FF.",
                                     camp_agregador="agr",
                                     agregar_data=FALSE, acumular = "env")%>%filter(FF.ANTIDIAB>90)
...
```{r generacio_dtindex4, include=F, eval=FALSE}
...
```{r lectura2, include=T, eval=FALSE}
...
saveRDS(dt_plana2, file=here::here(params$dir_dades_desti,"dt_plana2.rds"))
...
```{r GranFuncio,include=T, eval=FALSE}
