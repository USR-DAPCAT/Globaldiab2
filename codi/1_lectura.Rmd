...
dt_index_Glicada<-dt_variables %>% filter(cod=="GLICADA") %>% filter(val>=6.5) %>% group_by(idp) %>% slice(which.min(dat)) %>% ungroup() %>% transmute(idp,dtindex_HB=as.character(dat))
```
...
# Segons complicacions de la diabetis

```{r generacio_dtindex_SCRIP3, include=F, eval=TRUE}
...
dt_index_ComplicacioDM<-dt_diagnostics %>% 
...
dt_criteriDM<-dt_index_Glicada %>% 
  full_join(dt_index_FarmacsAD,by = "idp") %>% 
  full_join(dt_index_ComplicacioDM,by = "idp") %>%
  mutate(COUNT_HB2=dtindex_HB,COUNT_FX=dtindex_FX,COUNT_comp=dtindex_comp) %>% 
  mutate_at(vars( starts_with("COUNT_") ), funs( if_else(.==0  | is.na(.)  ,0,1))) %>% 
  mutate(N_condicions=COUNT_HB2+COUNT_FX+COUNT_comp) %>% 
  filter(N_condicions>=2) %>% 
  select(idp,dtindex_HB,dtindex_FX,dtindex_comp) %>% 
  mutate(CriteriDM=1)


# dt_index_FarmacsAD %>% filter(idp=="1faaebf01b43d2dc1121dae9b0f913d7e554f170")
# dt_criteriDM %>% filter(idp=="1faaebf01b43d2dc1121dae9b0f913d7e554f170")
# Fusió de tots els criteris
# dt_temp<-
#   mutate(data_min=pmin(dtindex_HB,dtindex_FX,dtindex_comp,na.rm = T)) %>% 
#   select(idp,dtindex =data_min)

# Fusió de tots els criteris i capturar data mes recent 
dt_temp<-
  dt_index_codiDM2 %>%  rename(dtindex_codDM=dtindex) %>% 
  full_join(dt_index_Glicada) %>% full_join(dt_index_FarmacsAD) %>% full_join(dt_index_ComplicacioDM) %>% 
  left_join(dt_criteriDM)


# Diabetic per Codi registrat o diabetic per criteris combinat (Fx o Hb o complicacions)
dt_diabetics<-dt_temp %>% filter(!is.na(dtindex_codDM) | CriteriDM==1)

```

# Data de debut

- Més recent entre totes les condicions
...
dt_temp3<-dt_prescripcions %>% semi_join(dt_codisAD,by="cod") %>% select(idp,dat) %>% group_by(idp) %>% slice_min(dat) %>% ungroup() %>% distinct() %>% 
   transmute(idp,dtindex=as.character(dat))
...
dt_temp<-
  dt_temp1 %>% bind_rows(dt_temp2) %>% bind_rows(dt_temp3) %>% bind_rows(dt_temp4) %>% bind_rows(dt_temp5) %>% 
...
# Genero tipus de diabetis segons CODI CIM10 (Etiqueta en HTC)
...
Dubtes: Que fem a la etiqueta de DM1/DM2/altres?
...
- No DM1
- Els que es detecta insunilització dins dels 2 anys post debut. 
...
dt_plana<-
  ...
  left_join(select(dt_diabetics,idp,DM2,DM1,DM_indeterminat,dtindex,ageDM,dies_insu)) %>%  
