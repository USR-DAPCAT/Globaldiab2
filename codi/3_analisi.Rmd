...
dades<-dades%>% filter(age>=15 & age<=39)
...
#dades<-dades%>%mutate(EVENT.MORT=ifelse(situacio=="2.   Difunto",sortida2,NA)) 
...
output_resum_event<-function(event = "EVENT.DFD") {
  ...
  dt_kk<-
    dt_temp %>% select(event:=sym(event),dtindex,sortida,sexe,age2.cat) %>% 
    ...
    dt_kk%>%
          group_by(sexe,age2.cat) %>% summarise(Patients = dplyr::n(), 
